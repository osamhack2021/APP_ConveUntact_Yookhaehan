...
class BasketballRezPage extends StatelessWidget {
  BasketballRezPage(this.date);

  BasketballController _controller = Get.find<BasketballController>();
  DateTime date;
      ...
      body: Column(
        children: [
          ...
          FieldRezHeader(
            today: date,
            facility: Facility(
              icon: teamFacility[1].icon,
              name: teamFacility[1].name,
              intro: teamFacility[1].intro,
            )
          ),
          _gridViewHeader(),
          Expanded(
                  ...
                  return Obx(() {
                    ...
                    _controller.endAbsTime.value != null) {
                      ...
                      if(state == RezState.ABLE) {
                        int cnt = _controller.basketballField.value.rezs!.length;
                        for(int i=0; i<cnt; i++){
                          BasketballRez rz = _controller.basketballField.value.rezs![i];
