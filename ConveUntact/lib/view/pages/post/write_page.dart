...
class WritePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    print("글쓰기 페이지 입니다.! 작성자는 : $email입니다.");
    return Scaffold(
      appBar: AppBar(),
      body: Padding(
                  ...
                  CustomElevatedButton(
                    ...
                    funpageRoute: () async{
                      if (_formKey.currentState!.validate()) {
                        bool pass = await add_post(email, _title.text, _content.text);
                        if(pass)
                          Get.off(HomePage(email:email)); // off 써서 뒤로 못돌아오도록 한다.
                        else
